  ...
  return (
    <main style={{ maxWidth: 780, margin: '40px auto', padding: 16 }}>
      ...
      >
        ...
        {/* 🔴 Eliminamos el botón de "Cerrar sesión" (ya está en el NavBar) */}
      </div>
      ...
      >
        <button onClick={computeHash} disabled={busy}>
          Generar hash
        </button>
        <button onClick={anchor} disabled={!hash || busy}>
          Anclar (local)
        </button>
        {hash && (
        ...
        {hash && (
          <a href={`/verify/${hash}`} target="_blank" rel="noreferrer">
            Verificar anclaje
          </a>
        )}
      </div>
